...
class OpenACC(TemplateView):
    """Annotate C99 source code with OpenACC compiler directives."""

    @method_decorator(csrf_exempt)
    def dispatch(self, request, *args, **kwargs):
        """To aviod checking the CSRF token when posting data to the server."""
        return super(OpenACC, self).dispatch(request, *args, **kwargs)

    def post(self,request,*args,**kwargs):
        """Parallize annotate the given file with OpenACC compiler directives.

        Creates a new file annotated with OpenACC directives, if the
        given file is parallelizable.
        """
        try:
            ...
            service = Service.objects.get(name='pragcc')
            resource = service.resource_set.get(name='openacc')
            ...
            data = {
                'message': (
                    "OpenACC resource does not exists in the data base"
                )
            }
